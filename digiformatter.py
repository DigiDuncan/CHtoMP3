...
def custom(name, message, showtime = True):
    ...
    if attr is not None:
        ...
        if showtime:
            print(time() + fg(fgcolor) + bg(bgcolor) + attr(attr) + message + style.RESET)
        else:
            print(fg(fgcolor) + bg(bgcolor) + attr(attr) + message + style.RESET)
...
def createLoadBar(current, total, barlength = 50, showpercent = False):
    ...
    returnstring = bar
    if showpercent:
        returnstring = f"{returnstring} {printableperc}%"
    return returnstring
